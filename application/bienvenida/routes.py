...
def obtener_productos_ganaderia(page):
    ...
    try:
        ...
        current_app.logger.error(f"{fecha_actual} Error al obtener productos: {str(e)}")
        return None


@bienvenida.route("/")
def index():
    """Función para información general"""
    data = {
        "titulo": "SETEAGROP S.A",
    }
    page = request.args.get('page', 1, type=int)
    productos_paginados = obtener_productos_ganaderia(page)
    print(productos_paginados)
    if productos_paginados is not None:
        return render_template("bienvenida/index.html", data=data, productos=productos_paginados)
    else:
        return render_template("error/error.html", mensaje="Error al obtener productos")


@bienvenida.route("/favicon.ico")
def favicon():
    """Función que devuelve un icono"""
    return send_from_directory(
        os.path.join(bienvenida.root_path, "static"), "favicon.ico", mimetype="image/x-icon"
    )