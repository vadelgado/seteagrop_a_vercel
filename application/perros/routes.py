...
@perros.route("/perros")
def perros_productos():
    """Función para mostrar los productos de gatos"""
    data = {
        "titulo": "Perros",
    }
    page = request.args.get('page', 1, type=int)
    productos_paginados = obtener_productos_perros(page)
    if productos_paginados is not None:
        return render_template("perros/perros.html", data=data, productos=productos_paginados)
    else:
